  ...
  return (
    <div className="flex-1 flex flex-col bg-white select-none">
      ...
      <div className="flex-1 px-5 py-3 space-y-3.5 overflow-y-auto flex flex-col">
        ...
        <div className="space-y-1"><h2 className="text-[20px] font-bold text-slate-900 tracking-tight" style={{ fontFamily: 'var(--font-heading)' }}>Acoustic Sweep</h2><p className="text-[13px] text-slate-600 leading-snug">Place the phone near the cylinder as it was during calibration. The app plays an audible 3-second high-frequency sweep and compares its frequency response locally.</p></div>
        ...
        {result && <div className={`rounded-2xl border p-4 space-y-3 ${resultTone === 'amber' ? 'bg-amber-50 border-amber-200' : resultTone === 'emerald' ? 'bg-emerald-50 border-emerald-200' : 'bg-sky-50 border-sky-200'}`}>
          ...
          {result.prediction === 'recheck' && <p className="text-xs text-amber-800 leading-snug">The saved fingerprints are too close for a reliable decision. Reposition the phone, reduce noise, and run the sweep again.</p>}
